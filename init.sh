...
[ -e "images/base-ide-ruby/build/Dockerfile" ] && rm images/base-ide-ruby/build/Dockerfile
...
sed \
  -e "s/\$TAG/$TAG/g" \
  -e "s/\$UID/$RANDOM/g" \
   < images/base-ide-ruby/build/Dockerfile.template > images/base-ide-ruby/build/Dockerfile
...
[ -e "images/base-ide-ruby/build/Dockerfile.build" ] && rm images/base-ide-ruby/build/Dockerfile.build
cp images/base-ide-ruby/build/Dockerfile images/base-ide-ruby/build/Dockerfile.build
cat >> images/base-ide-ruby/build/Dockerfile.build << EOP
LABEL io.codefresh.repo.owner=codefresh-io \\
           io.codefresh.repo.name=$MODULE \\
           io.codefresh.repo.branch=$(git symbolic-ref -q --short HEAD || git describe --tags --exact-match) \\
           io.codefresh.repo.sha=$(git rev-parse HEAD)
